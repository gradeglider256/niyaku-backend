...
export class CreatePermissionsSeed {
    async run(dataSource: DataSource): Promise<void> {
        ...
        const permissions = [
            { name: 'employee.add-employee', description: 'Can add new employees' },
            { name: 'employee.manage-employee', description: 'Can manage existing employees' },
            ...
            // Client permissions
            { name: 'clients.create', description: 'Can create new clients' },
            ...
            { name: 'clients.documents.create', description: 'Can add documents to clients' },
            { name: 'clients.addresses.create', description: 'Can add addresses to clients' },
            { name: 'clients.contacts.create', description: 'Can add contacts to clients' },
            { name: 'clients.documents.read', description: 'Can view documents of clients' },
            { name: 'clients.addresses.read', description: 'Can view addresses of clients' },
            { name: 'clients.contacts.read', description: 'Can view contacts of clients' },
        ];

        for (const perm of permissions) {
            const existing = await manager.findOne(Permission, { where: { name: perm.name } });
            if (!existing) {
